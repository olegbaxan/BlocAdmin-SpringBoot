...
public interface MeterDataRepository extends JpaRepository<MeterData, Integer> {
    ...
    @Query(value = "select max(m.current_value) FROM application.meterdata m inner join application.meterdata_meter mm on m.meterdataid=mm.meterdata_id inner join application.meters m2 on m.meterdataid =m2.meterid where m2.meterid=?1",nativeQuery = true)
